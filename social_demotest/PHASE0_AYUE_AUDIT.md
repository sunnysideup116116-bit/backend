...
> Public V2 的架構真相以 repository root 的 `AYUE_V2_ARCHITECTURE.md`
> 與 `AGENTS.md` 為準。
...
| public messaging | `/api/messages/*`, `/api/direct_chat*`, `/api/contacts` | 1,773–3,023 | 最後才拆；V2 仍委派 `ayue_agent.runtime` |
...
既有 `tests/test_ayue_agent_stream.py` 另固定 Public V2／legacy mode、owner 與
...
4. Public V2、private V2、legacy rollback、profile background task 與 stream
...
- `routers/chat.py` 仍是唯一掛入 `main.py` 的 aggregate router，include 四個 child
  routers；20 條 method/path/request surface 不變。Public V2 direct chat、private
...
- Manual fun-fact probe 與 pending probe answer 的 CAS／event state 已收斂至
  `relationship_engagement_service.py`；private runtime 與 private V2 不再 dynamic
  import `routers.chat`。
...
- `public_chat.py` 已承接 `POST /api/direct_chat` 與
  `POST /api/direct_chat/stream`，包括 public V2 的 save-once／progress stream
  orchestration 與僅限 `AYUE_AGENT_V2_MODE=off` 的 legacy rollback 路徑。
...
- Characterization tests 將兩條 direct-chat route 鎖定為
  ...
  V2 不載入 legacy rollback 的 deterministic tests 維持通過。
