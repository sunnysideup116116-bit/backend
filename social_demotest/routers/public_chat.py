...
def _complete_public_turn(
    ...
    history = list(messages_coll.find({"room_id": room_id}).sort("timestamp", -1).limit(20))[::-1]
...
def direct_chat(req: DirectChatRequest, background_tasks: BackgroundTasks):
    ...
    if mention_labels:
        user_metadata["mention_labels"] = mention_labels
    user_message = save_message(room_id, req.user_id, display_message, metadata=user_metadata or None)
