...
Planner (LLM, 拆 DAG)
  ...
  → Scheduler 注入 executor 參數 → execute_tool / prepare_write_confirmation
```

**核心定位**：

- **零 LLM**：Guard 不呼叫任何模型，不看語意、不猜意圖。它只做確定性（deterministic）檢查。
- **逐個 proposal 審核**：sub-agent 一次回覆可以產出多個 tool calls（例如同時查牛排與甜點），Scheduler 對**每一個** proposal 呼叫一次 `guard_proposal`，一個失敗不影響其他。
...
## 4. Guard 被拒絕之後（Scheduler 的處理）

`scheduler.py:_run_sub_task` 對每個 proposal：
...
Guard 通過後，Scheduler 還做三道確定性檢查（`scheduler.py`），也屬於「程式審核」範圍：
...
| `web.extract` URL 綁定 | URL 不是「本回合 web.search 結果或 owner 原句提供的公開 URL」 | `FAILED/web_extract_url_not_bound` |

## 6. 測試

`tests/test_v3_guard.py` 覆蓋全部 GuardResultCode：pass、unknown tool、forbidden arg field（validator 層）、schema invalid、duplicate、step limit、write requires confirmation。新增任何 Guard 規則時必須同步補測試。
