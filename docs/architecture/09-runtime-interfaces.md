...
- 2026-09-13：shared opening 正常路徑改用 `pair_opening_service` 的單次 bounded function call，輸出 `opening`、`question`、`evidence_key`、`evidence_quote`，驗證長度、單一問句及引用依據。輸入只含公開提案的 bounded evidence，不含使用者 ID、姓名、raw match、private opening 或 memory。姓名以 `proposal_display_name` 從 Appwrite-first 來源取得後再代入；缺名時不產生「對方、對方」。`opening_version=v2_personalized`／`opening_outcome` 僅是訊息 metadata，沒有新的 API authority。由既有 BackgroundTasks 在接受後生成，已存在的同 event-key 訊息不改写、不重生成；無資料／provider failure 保留安全 fallback。
- 暱稱 adapter 對已知 loopback HTTP 預設／80 埠 `/v1` 入口改用 App 既有固定 Appwrite HTTPS origin，避開 301；其他自訂 HTTPS／開發埠不變。仍禁止 credential-bearing URL、query／fragment 與任意 redirect，保留 TLS verification、name-only query 及 bounded cache。
