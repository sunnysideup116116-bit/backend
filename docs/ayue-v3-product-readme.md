...
| 路徑 | 用途 |
| --- | --- |
| `social/` | FastAPI 主服務、Web Demo、Public/Private Ayue 與測試（port 8000） |
...
```powershell
py -3.11 -m venv .local-venv
.\.local-venv\Scripts\python.exe -m pip install -r .\social\requirements.txt
.\.local-venv\Scripts\python.exe -m pip install -r .\matchmaker_agent\requirements.txt
```

分別複製兩個服務的環境範例後填入自己的值：

```powershell
Copy-Item .\social\.env.example .\social\.env
...
..\.local-venv\Scripts\python.exe -m unittest discover -s tests -p "test_*.py"
```
...
```powershell
Set-Location .\social
..\.local-venv\Scripts\python.exe -m compileall -q .
```
...
```powershell
Set-Location .\social
..\.local-venv\Scripts\python.exe scripts\cleanup_invalid_current_context.py
..\.local-venv\Scripts\python.exe scripts\repair_directional_match_reasons.py
```
