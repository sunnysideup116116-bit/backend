...
公開稱呼來源統一為 `public_nickname_service.contact_display_name`（Appwrite-first、Mongo 安全 fallback），涵蓋已接受聯絡人、名字解析、約會／通知及本人稱呼。批次查名只使用原流程已授權的 ID；缺名以 `name_available`／`names_complete` 明示，不從缺名推論未配對。匿名提案／redaction 保持原來的隱私語意。詳見 [`PUBLIC_NICKNAME_CONTRACT.md`](./PUBLIC_NICKNAME_CONTRACT.md)。
