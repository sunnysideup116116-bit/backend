...
- `social` 是產品後端與前端；`matchmaker_agent` 是候選排序及 Neo4j 記憶服務。
...
4. 只做任務需要的修改。不要格式化整份 `social/routers/chat.py` 或 `social/frontend.html`。
...
- `social/services/ayue_agent/v3/scheduler.py` 是公開阿月 V3 的唯一 orchestrator。
...
- 系統產生的長期建議是 recommendation，不是使用者事實。必須使用獨立 versioned contract、TTL、來源與 dismissed state，禁止寫成 `PREFERS`、`AVOIDS` 或 `CURRENTLY_WANTS`。
