...
class MobileProfileBootstrapContractTests(unittest.TestCase):
    ...
    def test_service_is_idempotent_and_preserves_completed_profile(self):
        ...
        self.assertIn('ensure_profile(profiles_coll, user_id)', source)
        boundary = (SERVER_ROOT / "social/services/profile_writer.py").read_text()
        self.assertIn('{"$setOnInsert": fields}', boundary)
        self.assertIn('fields["user_id"] = owner', boundary)
