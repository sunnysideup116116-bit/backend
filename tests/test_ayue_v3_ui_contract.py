...
class AyueV3UiContractTests(unittest.TestCase):
    ...
    def test_date_form_uses_notes_and_google_places_autocomplete(self):
        ...
        self.assertIn('dateTextarea("地點", "location"', source)
        self.assertIn("`${resolvedName}\\n${details}`", source)
        self.assertIn('id="date-form-location"', source)
        self.assertNotIn("約會活動 / 地點", source)
        self.assertNotIn('const activityInput = card.querySelector(\'[data-date-field="activity"]\')', source)
