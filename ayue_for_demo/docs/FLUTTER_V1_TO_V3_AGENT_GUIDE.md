...
3. 將 copied V1 的 `social_demotest/`、`matchmaker_agent/` 與啟動設定視為一個可替換子系統。
...
- 直接使用本 repo 的 `social_demotest/` 與 `matchmaker_agent/`，不要從 V1 router 逐段貼回。
...
子系統，以現行 V3 social_demotest + matchmaker_agent 建立新 baseline。
