...
- 第二階段以 `VOICE_APP_ASYNC_TOOLS_ENABLED` 控制可回滾的非同步工具；目前安全 allowlist 只有 `read_weather` 與 `ask_app_ayue`。天氣查詢會脫離 Live 接收迴圈執行，阿月委派保留既有背景 action/task 流程；所有寫入、確認、導航與其他工具仍固定 `BLOCKING`。
...
3. 使用者可以繼續手動修改文字。
4. 使用者可手動選擇 1–5 張圖片，或請語音阿月依最新排序與序號選取。
...
- 支援「幫我選相簿最近三張照片」、「選最新兩張」等張數指令，限制 1–5 張。
- 支援「選第三張圖片」或多個明確序號；序號依相簿最新到最舊計算，單項可指定第 1–20 張。
- 若目前不在發文頁，會先開啟既有發文草稿頁、恢復帳號本機草稿，再選取照片。
- Android 直接透過 `photo_manager` 讀取使用者已授權範圍內的「所有照片」最近項目；第一次仍須由使用者處理系統相簿權限畫面。
- 圖片路徑、縮圖與內容不會傳到語音 Server 或 Gemini；Server 只知道使用者要求的張數或相對序號，以及選取成功與否。
