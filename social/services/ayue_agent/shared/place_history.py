"""Durable server-owned history for presented Places candidates.
...
an immutable snapshot keyed by the originating public-agent run.  The old
...
def _warn_memory_fallback(reason: str) -> None:
    ...
    _LOGGER.warning(
        "Place presentation persistence is using process-local memory only "
...
def ensure_indexes() -> None:
    ...
    try:
        ...
        _LOGGER.warning("Unable to create place presentation indexes: %s", type(exc).__name__)
...
def get_candidate_set(
    ...
    *,
    ...
    try:
        ...
        _LOGGER.error(
            "Unable to load place presentation user=%s room=%s reason=%s",
...
def _active_followup_origin(user_id: str, room_id: str) -> str:
    """DAG place drafts are retired and no longer contribute hidden state."""
    del user_id, room_id
    return ""
...
def clear_candidate_set(user_id: str, room_id: str) -> None:
    ...
    try:
        ...
        _LOGGER.warning("Unable to clear place presentation snapshots: %s", type(exc).__name__)
