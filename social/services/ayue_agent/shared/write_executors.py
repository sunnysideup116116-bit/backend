"""Shared write executors: the only path that performs confirmed side effects.
...
from services.ayue_agent.public_relationship_projection import (
    ...
    search_accepted_contact_candidates,
)
...
def _start_search(
    ...
    try:
        ...
        result = start_match_search(
            ctx.user_id, source="agent_pi", force_new=True, idempotency_key=key,
    ...
    *, confirmation_id: str | None,
    ...
    try:
        result = cancel_match_search(
            ctx.user_id, source="agent_pi", expected_job_id=expected_job_id,
        )
...
def _prepare_date_coordination(
    ...
    if len(mention_ids) == 1:
        ...
        search = search_accepted_contact_candidates(ctx.user_id, evidence_span)
        exact = [item for item in search.candidates if item.kind == "exact"]
        if len(exact) == 1:
            target_id = exact[0].other_id
            safe_label = exact[0].display_name
            resolution_kind = "exact"
        if not target_id and search.status == "found" and search.candidates:
            return {
                "action": "relationship.select_date_invitation_contact",
                "arguments": {},
                "data": {
                    "name_hint": evidence_span[:120],
                    "candidates": [
                        {
            ...
            return None, _contact_resolution_failure(search.status, name_hint=evidence_span)
    else:
        return None, _contact_resolution_failure("not_found")

    if not target_id:
        return None, _contact_resolution_failure("not_found")
    return _prepare_date_coordination_for_contact_id(
        ctx, target_id, safe_label=safe_label, resolution_kind=resolution_kind,
    )


def _prepare_date_coordination_for_contact_id(
    ctx: Any, target_id: str, *, safe_label: str = "", resolution_kind: str = "exact",
) -> tuple[dict[str, Any] | None, str | None]:
    """Revalidate one Server-bound contact before preparing an invitation."""
    from services.date_coordination_service import LIVE_STATUSES, find_accepted_match

    try:
        current_label = relationship_display_name(target_id)
    except Exception:
        current_label = ""
    safe_label = (
        current_label if current_label and current_label != "對方"
        else safe_label or "對方"
    )[:30]
    try:
...
def _execute_calendar_mutation_plans(
    ...
    *,
    ...
    from .calendar_state import remember_recent_mutation
...
def execute_write(
    ...
    *,
    ...
    stable_operation_id = str((payload or {}).get("_operation_item_id") or "").strip()
    if tool_name.startswith("calendar."):
        cid = stable_operation_id or (payload or {}).get("_confirmation_id") or confirmation_id or uuid.uuid4().hex
    ...
    cid = stable_operation_id or (payload or {}).get("_confirmation_id") or confirmation_id
