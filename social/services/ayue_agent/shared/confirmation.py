"""Preview-bound confirmation manager shared by public Pi and private Ayue."""
...
def project_match_choice_history(messages: list[dict], *, user_id: str, room_id: str, collection: Any) -> list[dict]:
    ...
    try:
        ...
        }, {"_id": 1, "tool_name": 1, "arguments": 1, "payload.delivery_mode": 1,
            "status": 1, "selected_choice": 1, "expires_at": 1, "preview_text": 1})
        projections = {}
        for record in rows:
...
class ConfirmationManager:
    ...
    def create_confirmation(
        ...
        document = {
            ...
            "payload": safe_payload,
            "source_engine": (
                "private_v2" if surface == SURFACE_PRIVATE else "pi"
            ),
            "tool_protocol_version": str(
                safe_payload.get("tool_protocol_version") or "legacy"
            )[:80],
    ...
    def execute_confirmed(
        ...
        rec = actives[0]
        cid = rec["_id"]
        if surface == SURFACE_PUBLIC and rec.get("source_engine") != "pi":
            self._coll.update_one(
                {"_id": cid, "user_id": user_id, "status": {"$in": ["prepared", "pending"]}},
                {"$set": {
                    "status": "expired",
                    "resolved_at": time.time(),
