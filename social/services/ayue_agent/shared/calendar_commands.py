"""Typed Calendar commands and deterministic public-agent mutation preflight.
...
def _remember_candidate_projection(user_id: str, events: list[dict[str, Any]]) -> list[dict[str, str]]:
    """Return safe visible labels; Pi re-resolves canonical state next turn."""
    del user_id
    return [{"label": _event_label(event)} for event in events[:3]]
...
def _preflight_one(
    ...
    *,
    ...
    if resolution_kind == "fuzzy_suggestion":
        ...
        # authority for a destructive mutation. Ask using a visible label and
        # re-resolve canonical state from the owner's next turn.
