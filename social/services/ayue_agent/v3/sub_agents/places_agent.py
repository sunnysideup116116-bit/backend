...
行為規則：
- `place_mode` 是 server-validated typed intent：discover 才建立新候選清單；details／reviews 只處理 Planner 已在 task_brief 解出的具體店名與已知地址／區域。執行時用 `places.resolve_place` 重新核對，details／reviews 不得提出 `places.search_nearby`；reviews 不查距離。
...
- discover 有明確地點就使用該 anchor；沒有明確地點才可使用 saved location。details／reviews 若 task_brief 沒有具體店名，必須失敗澄清，不可用 saved location 或新搜尋結果猜一間。
...
- 使用者要求其他家／換一批／不要剛才的時，以 task_brief 中列出的可見舊店名作語意限制；不要使用 snapshot、opaque ref 或在 prompt 拼店家 ID。新地區、新類型或明確重新開始時保持 false。
